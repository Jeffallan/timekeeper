    ...
    <div>
        ...
        <b-table striped :items="this.data.items" :fields="this.data.fields">
            <template #cell(name)="d">
                <router-link :to="{ name: 'LocationsClientDetail', params: {id: d.item.id, type: 'client'} }" replace>
                    {{d.item.name}}
                </router-link>
            </template>
...
import { CLIENTS } from "@/util/constants/Urls.js"
...
export default {
    ...
    methods: {
        handleClick() {
            Router.push({name:"LocationsClientCreate", params: {type: "client"}})
        }
    },
}
</script>
